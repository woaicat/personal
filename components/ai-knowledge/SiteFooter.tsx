...
function SmartLink({ item }: { item: SiteLink }) {
  ...
  return <Link href={knowledgeRoute(item.href)}>{label}</Link>;
}
    ...
    <footer id="resources" className="site-footer">
      <div className="footer-grid">
        ...
        <section className="footer-col" aria-labelledby="footer-archive">
          ...
          <Link className="archive-link" href={knowledgeRoute(`${AI_KNOWLEDGE_BASE_PATH}#articles`)}>
            查看更多
            <span aria-hidden="true">→</span>
          </Link>
        </section>

        <img className="footer-image" src={`${AI_KNOWLEDGE_BASE_PATH}/images/article-storm-lake.png`} alt="" />
      </div>
